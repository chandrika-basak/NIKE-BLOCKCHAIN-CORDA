 ...
 */
object RunModifyFlow {
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val runID: String, val runnerPosition: String) : FlowLogic<SignedTransaction>() {
        /**
        ...
        override fun call(): SignedTransaction {
            ...
            val exp1 = builder { RunState.RunSchemaV1.RunEntity::runID.equal(runID) }
            val runCriteria = QueryCriteria.VaultCustomQueryCriteria(exp1)
            val unconsumedCriteria = QueryCriteria.VaultQueryCriteria(Vault.StateStatus.UNCONSUMED)
            val unconsumedSkuResponseStateAndRef = serviceHub.vaultService.queryBy<RunState>(runCriteria.and(unconsumedCriteria)).states.singleOrNull()
            val recipient = unconsumedSkuResponseStateAndRef!!.state.data.recipient


            val outRunModel = unconsumedSkuResponseStateAndRef!!.state.data.runModel


            var copyRunModel = outRunModel.copy()
            copyRunModel.runnerPosition = runnerPosition
            val signList = listOf<Party>(serviceHub.myInfo.legalIdentities.first())+ recipient

            // Stage 1.
            progressTracker.currentStep = GENERATING_TRANSACTION
            // Generate an unsigned transaction.
            val runState = RunState(copyRunModel, serviceHub.myInfo.legalIdentities.first(),recipient)
            val txCommand = Command(SkuTransferContract.Commands.ModifyRun(), signList.map { it.owningKey })
            val txBuilder = TransactionBuilder(notary).withItems(unconsumedSkuResponseStateAndRef,StateAndContract(runState, SkuTransferContract.SKU_TRANSFER_CONTRACT_ID), txCommand)
